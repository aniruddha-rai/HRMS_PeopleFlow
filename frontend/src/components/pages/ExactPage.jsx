...
import { getPageHtml } from '../../utils/exactFragments';
...
function ensureRoleAwareTeamOverrides() {
  ...
  window.renderAttTeamData = function renderAttTeamData() {
    ...
    if (monthlyRow) {
      ...
      monthlyRow.parentElement && (monthlyRow.parentElement.style.display = 'table-row-group');
    }
  };
}

function initAttendancePage(currentRole) {
  ...
  if (typeof window.scopeSwitch === 'function') {
    const meTab = document.querySelector('#attendanceScopeTabs .tab[data-scope="me"]');
    if (meTab) window.scopeSwitch('att', 'me', meTab);
  }
...
function initLeavePage(currentRole) {
  ensureRoleAwareTeamOverrides();
  const canUseTeam = currentRole === 'manager' || currentRole === 'hr';
  const scopeTabs = document.getElementById('leaveScopeTabs');
  if (scopeTabs) scopeTabs.style.display = canUseTeam ? 'block' : 'none';

  const approvalTab = document.getElementById('leaveApprovalTab');
  if (approvalTab) approvalTab.style.display = canUseTeam ? 'block' : 'none';
  ...
  if (typeof window.scopeSwitch === 'function') {
    const meTab = document.querySelector('#leaveScopeTabs .tab[data-scope="me"]');
    if (meTab) window.scopeSwitch('leave', 'me', meTab);
  }
}



function initTasksPage(currentRole) {
  ...
  if (approvalContent) approvalContent.style.display = canApprove ? approvalContent.style.display || 'none' : 'none';
...
function initPeoplePage(currentRole) {
  const isHR = currentRole === 'hr';
...
export function ExactPage({ pageId, currentRole }) {
  useEffect(() => {
    let cancelled = false;
    let tries = 0;
    ...
    function boot() {
      if (cancelled) return;
      if (pageId !== 'attendance' && pageId !== 'leave' && pageId !== 'people' && pageId !== 'tasks') return;
      if (pageId !== 'people' && pageId !== 'tasks' && typeof window.populateTeamDropdowns !== 'function') {
        tries += 1;
        if (tries < 20) window.setTimeout(boot, 100);
        return;
      }
      try {
        if (pageId === 'attendance') initAttendancePage(currentRole);
        if (pageId === 'leave') initLeavePage(currentRole);
        if (pageId === 'people') initPeoplePage(currentRole);
        if (pageId === 'tasks') initTasksPage(currentRole);
      } catch (error) {
        console.error(pageId + ' page init failed', error);
      }
    }
  ...
  return (
    <div className="page active" id={`page-${pageId}`} dangerouslySetInnerHTML={{ __html: getPageHtml(pageId) }} />
  );
}
